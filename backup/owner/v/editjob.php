...
<?php include('/home/aidifxin/cloud/dispatch/m/jobsAPI/editjob_api.php');?> 
<?PHP include($nav);?>
...
$resul=mysqli_query($conn,"SELECT * FROM jobs WHERE taxi_comp='$api' AND job_id='$id'");
while($res2 = mysqli_fetch_array($resul)) {
		                                
    $jobId = $res2['job_id'];	                              
	$phonelines = $res2['phonelines'];
    ...
    $flight_no=$res2['flight_no'];
    $assign_car=$res2['assign_car'];
    $comment=$res2['comment'];
    $distance_charge=$res2['distance_charge'];
    $fmile_charge=$res2['fmile_charge'];
    $api_key=$_SESSION['id'];
...
<h4>Dispatch Saved Jobs</h4>
  ...
  <div>
  <p align="right"><input type="text-half" class="jobentry-half" name="job_id" value="<?php echo $res2['job_id'];?>" placeholder="Job#" /></p>
    ...
    <label class="">Cross Street:</label><br>
     <input class="input-half" type="text" placeholder="Cross Street:" name="cross_street" value="<?php echo $cross_street;?>"><br>
      ...
      <label>Selected Driver:</label> <br>
          <?php echo include('/home/aidifxin/cloud/dispatch/m/driverAPI/selectAdriverAPI.php');?>
       <br>
